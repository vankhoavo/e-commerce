...
class OrderController extends Controller
{
    ...
    public function store(Request $request, DatabaseManager $db): JsonResponse
    {
        ...
        try { Mail::to($recipient)->send(new OrderReceivedMail($order)); }
        catch (\Throwable $exception) { Log::warning('Không thể gửi Email xác nhận đơn hàng.', ['order_id' => $order->id, 'recipient' => $recipient, 'error' => $exception->getMessage()]); }
