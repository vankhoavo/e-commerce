...
type PaypalSdk = {
    Buttons: (options: Record<string, any>) => PaypalButtons;
};
...
function loadPaypalSdk(): Promise<void> {
    ...
    if (!paypalClientId) return Promise.reject(new Error('Thiếu PayPal Sandbox Client ID.'));
...
async function renderPaypalButtons() {
    if (paypalRendered.value || !paypalContainer.value || !paypalReady.value || !window.paypal || form.value.payment !== 'paypal') return;
    ...
    try {
        await window.paypal.Buttons({
            ...
            onApprove: async (data: { orderID: string }) => {
                ...
                try {
                    ...
                    saveOrder('paypal-sandbox', data.orderID);
                } catch (error) {
                    paypalError.value = error instanceof Error ? error.message : 'Không thể hoàn tất PayPal Sandbox.';
                } finally {
            ...
            onCancel: () => {
                paypalLoading.value = false;
                paypalError.value = 'Bạn đã hủy cửa sổ thanh toán PayPal Sandbox.';
            },
        ...
        paypalError.value = error instanceof Error ? error.message : 'Không thể tải PayPal Sandbox.';
    }
}

async function enablePaypal() {
    if (form.value.payment !== 'paypal') return;
    paypalError.value = '';
    if (!paypalClientId) {
        paypalError.value = 'TechStore chưa có PayPal Sandbox Client ID. Hãy thêm VITE_PAYPAL_CLIENT_ID vào .env.';
        return;
    }
    try {
        await loadPaypalSdk();
...
function selectPayment(payment: string) {
    ...
    if (payment === 'paypal') {
        paypalRendered.value = false;
        nextTick(enablePaypal);
    }
}

watch(() => form.value.payment, (payment) => {
    if (payment === 'paypal') nextTick(enablePaypal);
});
    ...
    <div class="checkout-page">
        <div class="container checkout-container">
            ...
            <section v-if="submitted" class="checkout-success">
                ...
                <div class="sandbox-success" v-if="form.payment === 'paypal-sandbox'"><i class="bi bi-shield-check"/><span>Thanh toán đã được xác nhận trên <strong>PayPal Sandbox</strong>. Đây là giao dịch thử nghiệm, không chuyển tiền thật.</span></div>
                <Link href="/products" class="success-btn">Tiếp tục mua sắm <i class="bi bi-arrow-right"/></Link>
            </section>

            <template v-else-if="cart.length">
                <div class="checkout-heading"><div><span class="checkout-kicker">TECHSTORE · CHECKOUT</span><h1>Đặt hàng &amp; vận chuyển</h1><p>Thông tin cá nhân đã lưu sẽ được tự động điền vào biểu mẫu.</p></div><span class="checkout-count"><i class="bi bi-bag"/> {{ itemCount }} sản phẩm</span></div>

                <form class="row g-4" @submit.prevent="submitCod">
                    <section class="col-lg-7">
                        <div class="checkout-card">
                            <div class="checkout-card-title"><span>1</span><div><strong>Thông tin nhận hàng</strong><small>Tên, số điện thoại và địa chỉ được lấy từ hồ sơ tài khoản.</small></div></div>
                    ...
                    <aside class="col-lg-5">
                        <div class="checkout-order-card">
                            ...
                            <div class="payment-block">
                                ...
                                <button type="button" class="payment-option" :class="{ active: form.payment === 'cod' }" @click="selectPayment('cod')">
                                    <span class="payment-option-icon cod-icon"><i class="bi bi-cash-stack"/></span><span class="payment-option-copy"><strong>Thanh toán khi nhận hàng</strong><small>COD · Không phát sinh thêm phí.</small></span><span class="payment-radio" :class="{ checked: form.payment === 'cod' }"/>
                                </button>
                                <button type="button" class="payment-option" :class="{ active: form.payment === 'paypal' }" @click="selectPayment('paypal')">
                                    <span class="payment-option-icon paypal-icon"><i class="bi bi-paypal"/></span><span class="payment-option-copy"><strong>PayPal Sandbox</strong><small>Môi trường thử nghiệm · Không chuyển tiền thật.</small></span><span class="payment-radio" :class="{ checked: form.payment === 'paypal' }"/>
                                </button>
                                ...
                                <div v-if="form.payment === 'paypal'" class="paypal-sandbox-panel">
                                    ...
                                    <div class="sandbox-amount"><span>Số tiền thử nghiệm</span><strong>$ {{ paypalUsd.toFixed(2) }} USD</strong><small>Quy đổi cấu hình thử nghiệm: 1 USD ≈ {{ VND_TO_USD.toLocaleString('vi-VN') }} VND</small></div>
                                    <div class="sandbox-info"><i class="bi bi-info-circle"/><span>Sandbox là môi trường riêng của PayPal để mô phỏng giao dịch. Bạn đăng nhập bằng <strong>tài khoản Sandbox Personal</strong>; không dùng tài khoản hay thẻ thật.</span></div>
                                    ...
                                    <p class="sandbox-footnote"><i class="bi bi-lock-fill"/> TechStore gọi PayPal Orders API ở phía máy chủ; trình duyệt không chứa Client Secret.</p>
                                </div>
                            </div>
