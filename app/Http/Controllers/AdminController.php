...
use App\Http\Requests\Admin\DeleteOrdersRequest;
...
class AdminController extends Controller
{
    ...
    public function orders():Response{return Inertia::render('admin/Orders',['orders'=>Order::query()->with(['items','user:id,name,email,phone'])->latest()->paginate(15)->withQueryString()]);}
    public function orderApprove(Order $order):RedirectResponse{abort_unless($order->status==='Chờ xử lý',422,'Đơn hàng không ở trạng thái chờ duyệt.');DB::transaction(function()use($order):void{$order->load('items');foreach($order->items as $item){if(!$item->product_id)continue;$product=AdminProduct::query()->lockForUpdate()->find($item->product_id);if(!$product)abort(422,'Sản phẩm không còn tồn tại: '.$item->name);if($product->stock<$item->quantity)abort(422,'Tồn kho không đủ cho sản phẩm: '.$product->name);$product->decrement('stock',$item->quantity);$product->increment('sold_count',$item->quantity);} $order->update(['status'=>'Đã duyệt']);});$order->refresh()->load('user','items');$recipient=$order->customer_email?:$order->user?->email;if($recipient){try{Mail::to($recipient)->send(new OrderProcessedMail($order));}catch(\Throwable $exception){Log::warning('Không thể gửi Email đơn hàng đã xử lý.',['order_id'=>$order->id,'recipient'=>$recipient,'error'=>$exception->getMessage()]);}}return back()->with('success','Đã duyệt đơn hàng, cập nhật tồn kho và gửi Email trạng thái xử lý.');}
    public function orderStatusUpdate(Request $request, Order $order): RedirectResponse
    {
        $data = $request->validate(['status' => ['required', Rule::in(['Đã duyệt', 'Đang giao', 'Đã giao', 'Hoàn tất'])]]);
        $newStatus = $data['status'];
        abort_unless($order->status !== 'Hủy hàng', 422, 'Đơn hàng đã hủy không thể cập nhật trạng thái.');
        $allowedTransitions = [
            'Chờ xử lý' => ['Đã duyệt'],
            'Đã duyệt' => ['Đang giao'],
            'Đang giao' => ['Đã giao'],
        ...
        abort_unless(in_array($newStatus, $allowedTransitions[$order->status] ?? [], true), 422, 'Không thể chuyển trạng thái đơn hàng theo quy trình.');
        $order->update(['status' => $newStatus]);
        $success = 'Đã cập nhật trạng thái đơn hàng thành '.$newStatus.'.';
        if ($newStatus === 'Hoàn tất' && $order->vat_invoice_requested && filled($order->vat_email)) {
            try {
                Mail::to($order->vat_email)->send(new VatInvoiceMail($order->refresh()->load('items')));
                $success .= ' Đã gửi Email hóa đơn VAT.';
            } catch (\Throwable $exception) {
                Log::warning('Không thể gửi Email hóa đơn VAT khi đơn hàng hoàn tất.', ['order_id' => $order->id, 'recipient' => $order->vat_email, 'error' => $exception->getMessage()]);
                $success .= ' Trạng thái đơn đã hoàn tất nhưng Email hóa đơn VAT chưa gửi được.';
            }
        }
        return back()->with('success', $success);
    }
    public function orderUpdate(Request $request,Order $order):RedirectResponse{$data=$request->validate(['customer_name'=>['required','string','max:255'],'customer_phone'=>['required','string','max:30'],'customer_email'=>['nullable','email','max:255'],'customer_address'=>['required','string','max:500'],'note'=>['nullable','string','max:5000']]);$order->update($data);return back()->with('success','Đã cập nhật thông tin người nhận của đơn hàng.');}
    public function orderDelete(DeleteOrdersRequest $request,Order $order):RedirectResponse{$this->validateAdminOrderDeletionPassword($request->string('password')->toString());DB::transaction(function()use($order):void{$order->items()->delete();$order->delete();});return to_route('admin.orders')->with('success','Đã xóa đơn hàng '.$order->code.'.');}
    public function ordersDeleteAll(DeleteOrdersRequest $request):RedirectResponse{$this->validateAdminOrderDeletionPassword($request->string('password')->toString());DB::transaction(function():void{DB::table('order_items')->delete();DB::table('orders')->delete();});return to_route('admin.orders')->with('success','Đã xóa toàn bộ đơn hàng.');}
    private function validateAdminOrderDeletionPassword(string $password):void{abort_unless(Hash::check($password,request()->user()->password),422,'Mật khẩu quản trị viên không chính xác.');}
    public function administrators():Response{return Inertia::render('admin/Administrators',['administrators'=>User::query()->where('role',UserRole::ADMIN->value)->latest()->paginate(20)->withQueryString(),'permissions'=>self::adminPermissionDefinitions()]);}
    public function administratorStore(Request $request):RedirectResponse{$data=$request->validate(['name'=>['required','string','max:255','regex:/^[^@\\s]+$/','unique:users,name'],'email'=>['required','email','max:255','unique:users,email'],'password'=>['required','string','min:8'],'admin_permissions'=>['nullable','array'],'admin_permissions.*'=>['string',Rule::in(array_keys(self::adminPermissionDefinitions()))]]);User::create([...$data,'role'=>UserRole::ADMIN->value,'is_active'=>true,'email_verified_at'=>now(),'google_id'=>null,'admin_permissions'=>array_values($data['admin_permissions']??[])]);return back()->with('success','Đã tạo tài khoản quản trị viên.');}
    public function administratorUpdate(Request $request,User $user):RedirectResponse{abort_unless($user->role===UserRole::ADMIN,404);$data=$request->validate(['name'=>['required','string','max:255','regex:/^[^@\\s]+$/',Rule::unique('users','name')->ignore($user->id)],'email'=>['required','email','max:255',Rule::unique('users','email')->ignore($user->id)],'password'=>['nullable','string','min:8'],'admin_permissions'=>['nullable','array'],'admin_permissions.*'=>['string',Rule::in(array_keys(self::adminPermissionDefinitions()))],'is_active'=>['required','boolean']]);if($user->name==='admin'){$data['name']='admin';$data['is_active']=true;$data['admin_permissions']=null;}if(array_key_exists('password',$data)&&blank($data['password']))unset($data['password']);$user->update($data);return back()->with('success','Đã cập nhật quản trị viên.');}
