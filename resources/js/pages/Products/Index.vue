...
import { writeCart, readCart, type CartItem } from '@/lib/cart';

type P = { id:number; slug:string; name:string; category:string; categorySlug:string; brand:string; price:number; oldPrice?:number|null; image?:string|null; badge?:string|null; rating:number; sold:number; stock:number; sku?:string|null; productType?:string|null; productLine?:string|null };
type C = { id:number; name:string; slug:string; icon:string|null; count:number };
type F = { brands:string[]; types:string[]; lines:string[] };

const props = defineProps<{ category?:string; search?:string; products:P[]; categories:C[]; filters:F }>();
const page = usePage();
...
const filtered = computed(() => {
  ...
  if (category.value !== 'all') a = a.filter(x => x.categorySlug === category.value);
  const s = q.value.trim().toLowerCase();
  if (s) a = a.filter(x => `${x.name} ${x.brand} ${x.category} ${x.productLine ?? ''} ${x.productType ?? ''} ${x.sku ?? ''}`.toLowerCase().includes(s));
  if (brand.value) a = a.filter(x => x.brand === brand.value);
  if (type.value) a = a.filter(x => x.productType === type.value);
  if (line.value) a = a.filter(x => x.productLine === line.value);
  if (min.value) a = a.filter(x => x.price >= Number(min.value));
  if (max.value) a = a.filter(x => x.price <= Number(max.value));
  if (sort.value === 'low') a = [...a].sort((x,y) => x.price - y.price);
  if (sort.value === 'high') a = [...a].sort((x,y) => y.price - x.price);
  if (sort.value === 'rating') a = [...a].sort((x,y) => y.rating - x.rating);
  return a;
});

const pages = computed(() => Math.max(1, Math.ceil(filtered.value.length / perPage)));
const shown = computed(() => filtered.value.slice((current.value - 1) * perPage, current.value * perPage));
const fmt = (n:number) => new Intl.NumberFormat('vi-VN', { style:'currency', currency:'VND', maximumFractionDigits:0 }).format(n);
const count = (key:string, value:string) => props.products.filter(x => String((x as any)[key] ?? '') === value).length;
...
function add(p:P) {
  const c = readCart(userId.value);
  const x = c.find(i => i.id === p.id);
  if (x) x.quantity = Math.min(p.stock, x.quantity + 1);
  else c.push({ id:p.id, name:p.name, price:p.price, image:p.image ?? '', quantity:1 } as CartItem);
  writeCart(userId.value, c);
}
</script>
  ...
  <main class="catalog-page">
    ...
    <header class="catalog-hero">
      <div>
        ...
        <p>Tìm theo thương hiệu, loại sản phẩm, dòng/series, khoảng giá hoặc mã sản phẩm.</p>
      </div>
    ...
    <div class="catalog-layout">
      <aside class="catalog-filters">
        ...
        <select v-model="type"><option value="">Tất cả loại</option><option v-for="t in filters.types" :key="t" :value="t">{{ t }} · {{ count('productType', t) }} dòng</option></select>
