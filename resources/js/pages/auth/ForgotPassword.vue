...
defineOptions({
    layout: {
        ...
        description: 'Xác minh Email bằng mã OTP để đặt lại mật khẩu.',
    },
});
    ...
    <div class="forgot-card">
        <div class="forgot-hero">
            ...
            <div>
                ...
                <p>Nhập Email đã đăng ký để nhận mã OTP xác minh.</p>
            </div>
        </div>
        ...
        <Form action="/forgot-password" method="post" v-slot="{ errors, processing }" class="forgot-form">
            <div>
            ...
            <button type="submit" class="forgot-submit" :disabled="processing">
                ...
                <i v-else class="bi bi-shield-check-fill" />
                {{ processing ? 'Đang gửi OTP...' : 'Gửi mã OTP' }}
            </button>
        </Form>

        <div class="forgot-note">
            <i class="bi bi-info-circle" />
            <span>OTP có hiệu lực trong 10 phút. Kiểm tra cả thư mục Spam hoặc Quảng cáo nếu chưa thấy Email.</span>
        </div>
...
.forgot-input input{width:100%;border:0;outline:0;color:#344054;background:transparent;font-size:11px}
...
.forgot-note{display:flex;align-items:center;gap:7px;margin-top:15px;padding:10px;border-radius:9px;color:#667085;background:#f8fafc;font-size:9px;line-height:1.5}
