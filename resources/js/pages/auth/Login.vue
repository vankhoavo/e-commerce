...
<a :href="`/auth/google`" class="btn btn-outline-dark w-100 auth-google-btn" aria-label="Đăng nhập bằng Google"><svg class="google-logo" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M21.35 12.27c0-.71-.06-1.4-.18-2.05H12v3.88h5.23a4.47 4.47 0 0 1-1.94 2.93v2.43h3.14c1.84-1.69 2.92-4.18 2.92-7.19z"/><path fill="#34A853" d="M12 21.82c2.63 0 4.84-.87 6.45-2.36l-3.14-2.43c-.87.58-1.98.92-3.31.92-2.54 0-4.69-1.72-5.46-4.03H3.3v2.51A9.8 9.8 0 0 0 2.25 12c0 1.59.38 3.1 1.05 4.43l3.24-2.51C7.31 7.77 9.46 6.05 12 6.05z"/><path fill="#FBBC05" d="M6.54 13.92A5.86 5.86 0 0 1 6.23 12c0-.67.11-1.32.31-1.92V7.57H3.3A9.8 9.8 0 0 0 2.25 12c0 1.59.38 3.1 1.05 4.43l3.24-2.51C7.31 7.77 9.46 6.05 12 6.05z"/><path fill="#EA4335" d="M12 6.05c1.43 0 2.71.49 3.72 1.46l2.79-2.79C16.83 3.07 14.63 2.18 12 2.18a9.74 9.74 0 0 0-8.7 5.39l3.24 2.51C7.31 7.77 9.46 6.05 12 6.05z"/></svg><span>Tiếp tục với Google</span></a>
...
<div><label for="email" class="form-label">Tài khoản / Email</label><input id="email" type="text" name="email" class="form-control form-control-lg" required autofocus autocomplete="username" placeholder="email@example.com"/><div v-if="errors.email" class="auth-login-error" role="alert"><i class="bi bi-exclamation-circle-fill"/><span>{{errors.email==='auth.failed'?'Tài khoản hoặc mật khẩu không đúng.':errors.email}}</span></div></div>
<div><label for="password" class="form-label mb-1">Mật khẩu</label><PasswordInput id="password" name="password" class="form-control form-control-lg" required autocomplete="current-password" placeholder="Mật khẩu"/><div class="forgot-under-password"><Link href="/settings/security/forgot-password"><i class="bi bi-key me-1"/>Quên mật khẩu?</Link></div><div v-if="errors.password" class="auth-login-error" role="alert"><i class="bi bi-exclamation-circle-fill"/><span>{{errors.password==='auth.failed'?'Tài khoản hoặc mật khẩu không đúng.':errors.password}}</span></div></div>
