...
<template>
    <div class="admin-page ip-page">
        ...
        <div class="source-note"><i class="bi bi-shield-check"/><span>Dữ liệu tra cứu IP được đối chiếu từ <strong>IPinfo</strong>, <strong>ipwho.is</strong> và <strong>Google DNS PTR</strong> khi nguồn cung cấp thông tin.</span></div>
                ...
                <table class="table ip-table align-middle">
                    ...
                    <tbody>
                        <tr v-for="row in props.logs.data" :key="row.id">
                            ...
                            <td><div v-if="row.ipv4" class="ip-cell"><strong>{{ row.ipv4 }}</strong><small>{{ sources(row)?.join(' + ') || 'IPinfo + ipwho.is + Google DNS PTR' }}</small></div><span v-else>—</span></td>
