...
const meta = (status: string) =>
  status === 'Đã giao'
    ? { icon: 'bi-check-circle-fill', c: 'done' }
    : status === 'Hủy hàng'
      ? { icon: 'bi-x-circle-fill', c: 'cancel' }
      : status.includes('trả') || status.includes('Trả') || status === 'Đã hoàn tiền'
        ? { icon: 'bi-arrow-return-left', c: 'return' }
        : status === 'Đang giao'
          ? { icon: 'bi-truck', c: 'shipping' }
          : { icon: 'bi-hourglass-split', c: 'pending' };
...
const payment = (value: string) =>
  value === 'paypal-sandbox' ? 'PayPal Sandbox' : 'Thanh toán khi nhận hàng';
...
async function submitReturn() {
  ...
  if (!returnReason.value.trim()) {
    actionError.value = 'Vui lòng nhập lý do trả hàng.';
    return;
  }

  try {
    const data = await request(`/orders/${selected.value.id}/return`, 'PATCH', {
      reason: returnReason.value.trim(),
      customer_note: returnNote.value.trim() || null,
    });
    ...
    if (index >= 0) {
      orders.value[index] = {
        ...orders.value[index],
        status: data.status ?? 'Yêu cầu trả hàng',
      };
      selected.value = orders.value[index];
    }
...
<template>
  <section class="orders-page">
    <header class="head">
      <div>
        <span>TECHSTORE · TÀI KHOẢN</span>
        <h2>Đơn hàng</h2>
        <p>Lịch sử mua hàng, hóa đơn và đổi trả.</p>
      </div>
    ...
    <div v-if="filtered.length" class="list">
      <article v-for="order in filtered" :key="order.id" class="order-card">
        <header>
          ...
          <span :class="['status', meta(order.status).c]">
            <i :class="['bi', meta(order.status).icon"></i>
            {{ order.status }}
          </span>
        </header>
        ...
        <div class="body">
          ...
          <div class="summary">
            <span>{{ order.items.reduce((count, item) => count + item.quantity, 0) }} sản phẩm · {{ payment(order.payment) }}</span>
        ...
        <footer>
          <span>
            ...
            {{ order.vatInvoice.requested ? 'Hóa đơn VAT' : 'Hóa đơn bán hàng' }}
          </span>
          <div>
            <a
            ...
            <button
              ...
              class="return"
              @click="open(order); returnOpen = true"
            >
              Trả hàng
            </button>
          </div>
    ...
    <div v-else class="empty">
      ...
      <p>
        {{
          filter === 'all'
            ? 'Các đơn hàng sau khi đặt sẽ xuất hiện tại đây.'
            : 'Hãy chọn một trạng thái khác để xem đơn hàng.'
        }}
      </p>
    </div>
        ...
        <section class="modal">
          ...
          <div class="modal-body">
            <div class="detail-status">
              <span :class="['status', meta(selected.status).c]">{{ selected.status }}</span>
              <strong>{{ formatPrice(selected.total) }}</strong>
            </div>

            <section>
              <h3>Thông tin nhận hàng</h3>
              <p><b>{{ selected.customer?.name }}</b> · {{ selected.customer?.phone }}</p>
            ...
            <section>
              ...
              <p>
                {{ payment(selected.payment) }} · Tạm tính {{ formatPrice(selected.subtotal) }} ·
                Vận chuyển {{ formatPrice(selected.totalShipping) }}
              </p>
              <p v-if="selected.vatInvoice.requested">
                VAT: {{ selected.vatInvoice.companyName }} · MST {{ selected.vatInvoice.taxCode }}
              </p>
            </section>
            ...
            <div v-if="returnOpen" class="return-box">
              ...
              <textarea
                v-model="returnNote"
                rows="2"
                placeholder="Ghi chú cho Bán hàng"
              ></textarea>
          ...
          <footer>
            ...
            <a
              :href="`/orders/${selected.id}/invoice`"
              target="_blank"
              rel="noopener"
              class="invoice"
            >
            ...
            <button
              v-if="selected.status === 'Đã giao' && !returnOpen"
              class="return"
