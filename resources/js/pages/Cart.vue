...
function loadCart() {
    try {
        ...
        cart.value = Array.isArray(raw)
            ? raw
                .filter((item) => item && Number(item.id) && Number(item.quantity) > 0)
                .map((item) => ({
                    id: Number(item.id),
...
function syncCart() {
    localStorage.setItem('techstore_cart', JSON.stringify(cart.value));
...
function setQuantity(item: CartItem, value: string | number) {
    const stock = getStock(item.id);
    const quantity = Math.max(1, Math.min(stock, Math.floor(Number(value) || 1)));
...
const shipping = computed(() => subtotal.value > 0 ? 0 : 0);
...
<template>
    ...
    <div class="cart-page">
        <div class="container cart-container">
            <nav class="cart-breadcrumb" aria-label="Đường dẫn">
                <Link href="/">Trang chủ</Link>
                <i class="bi bi-chevron-right" aria-hidden="true" />
                <span>Giỏ hàng</span>
            </nav>

            <div v-if="notice" class="cart-notice" role="status">
            ...
            <div class="cart-heading">
                <div>
                    <span class="cart-kicker">TECHSTORE · SHOPPING CART</span>
                    <h1>Giỏ hàng</h1>
                    <p v-if="itemCount">Bạn đang có <strong>{{ itemCount }}</strong> sản phẩm trong giỏ hàng.</p>
                    <p v-else>Kiểm tra sản phẩm trước khi tiến hành đặt hàng.</p>
                </div>
                <button v-if="cart.length" type="button" class="clear-cart-btn" @click="clearCart">
                    <i class="bi bi-trash3" /> Xóa giỏ hàng
                </button>
            </div>
            ...
            <div v-if="cart.length" class="row g-4 align-items-start">
                    ...
                    <div class="cart-card">
                        ...
                        <article v-for="item in cart" :key="item.id" class="cart-item">
                            <div class="cart-product-image">
                                ...
                                <span class="cart-image-fallback"><i class="bi bi-laptop" /></span>
                            </div>

                            <div class="cart-product-info">
                                <div class="cart-product-brand">{{ products.find((product) => product.id === item.id)?.brand ?? 'TECHSTORE' }}</div>
                                <Link :href="`/products/${products.find((product) => product.id === item.id)?.slug ?? ''}`" class="cart-product-name">
                                    {{ item.name }}
                                </Link>
                            ...
                            <div class="cart-quantity">
                                <button type="button" aria-label="Giảm số lượng" @click="decrease(item)">−</button>
                                <input :value="item.quantity" type="number" min="1" :max="getStock(item.id)" aria-label="Số lượng" @change="setQuantity(item, ($event.target as HTMLInputElement).value)" />
                                <button type="button" aria-label="Tăng số lượng" @click="increase(item)">+</button>
                            </div>

                            <div class="cart-line-total">{{ formatPrice(item.price * item.quantity) }}</div>

                            <button type="button" class="remove-cart-btn" title="Xóa sản phẩm" aria-label="Xóa sản phẩm" @click="removeItem(item.id)">
                                <i class="bi bi-trash3" />
                            </button>
                        </article>

                        <div class="cart-card-footer">
                            <Link href="/products" class="continue-shopping"><i class="bi bi-arrow-left" /> Tiếp tục mua sắm</Link>
                        </div>
                    </div>
                </section>
                ...
                <aside class="col-lg-4">
                    <div class="cart-summary-card">
                        ...
                        <div class="summary-row"><span>Phí vận chuyển</span><strong class="free-shipping">Miễn phí</strong></div>
                        <div class="summary-divider" />
                        <div class="summary-total"><span>Tổng cộng</span><strong>{{ formatPrice(total) }}</strong></div>
                        <p class="summary-note"><i class="bi bi-shield-check" /> Giá đã bao gồm thuế theo chính sách TechStore.</p>
                        <button type="button" class="checkout-btn" disabled>
                            Tiến hành thanh toán <i class="bi bi-arrow-right" />
                        </button>
                        <small class="checkout-coming">Thanh toán sẽ được kích hoạt ở bước Checkout.</small>
                    </div>

                    <div class="cart-service-card">
                        <div><i class="bi bi-truck" /><span><strong>Giao hàng toàn quốc</strong><small>Đóng gói an toàn, theo dõi đơn.</small></span></div>
                        <div><i class="bi bi-arrow-repeat" /><span><strong>Đổi trả thuận tiện</strong><small>Hỗ trợ theo chính sách TechStore.</small></span></div>
                        <div><i class="bi bi-headset" /><span><strong>Hỗ trợ mua hàng</strong><small>Tư vấn trước và sau khi đặt hàng.</small></span></div>
                    </div>
                </aside>
            </div>

            <section v-else class="empty-cart-card">
                <div class="empty-cart-icon"><i class="bi bi-bag-x" /></div>
                <span class="cart-kicker">SHOPPING CART</span>
                <h2>Giỏ hàng đang trống</h2>
                <p>Hãy khám phá laptop, linh kiện và phụ kiện để chọn sản phẩm phù hợp.</p>
                <Link href="/products" class="empty-cart-btn">Khám phá sản phẩm <i class="bi bi-arrow-right" /></Link>
            </section>
        </div>
    </div>
</template>
