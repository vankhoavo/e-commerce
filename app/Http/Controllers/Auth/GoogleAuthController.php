...
class GoogleAuthController
{
    ...
    public function callback(Request $request): RedirectResponse
    {
        ...
        // Google UserInfo does not contain phone/birthday. People API may return
        // them when the user grants the requested scopes and has the data set.
        $people = Http::withToken($accessToken)
            ->get('https://people.googleapis.com/v1/people/me', ['personFields' => 'phoneNumbers,birthdays'])
            ->json();
        $phone = data_get($people, 'phoneNumbers.0.canonicalForm') ?: data_get($people, 'phoneNumbers.0.value');
        $birth = data_get($people, 'birthdays.0.date');
        $birthDate = null;
        if (is_array($birth) && data_get($birth, 'year') && data_get($birth, 'month') && data_get($birth, 'day')) {
            $birthDate = sprintf('%04d-%02d-%02d', $birth['year'], $birth['month'], $birth['day']);
        }
