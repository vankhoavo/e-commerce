    ...
    <nav class="shop-navbar navbar navbar-expand-lg sticky-top">
        <div class="container nav-container">
            ...
            <div :class="['navbar-collapse', 'client-navbar-collapse', mobileMenuOpen ? 'show' : '']">
                <div class="nav-primary-links"><Link href="/" class="shop-nav-link">Trang chủ</Link><Link href="/products?category=laptop" class="shop-nav-link">Laptop</Link><Link href="/products?category=components" class="shop-nav-link">Linh kiện</Link><Link href="/products?category=monitor" class="shop-nav-link">Màn hình</Link><Link href="/products?category=accessories" class="shop-nav-link">Phụ kiện</Link></div>
                <form class="nav-search" @submit.prevent="submitSearch"><i class="bi bi-search nav-search-icon"/><input v-model="search" type="search" placeholder="Tìm laptop, linh kiện..." aria-label="Tìm sản phẩm"/><button type="submit" class="nav-search-submit"><i class="bi bi-arrow-right"/><span>Tìm</span></button></form>
                <div class="nav-actions">
                    <Link href="/cart" class="nav-cart d-none d-lg-flex position-relative" aria-label="Giỏ hàng"><span class="nav-cart-icon"><i class="bi bi-bag"/></span><span class="nav-cart-label"><strong>Giỏ hàng</strong><small>{{ cartCount }} sản phẩm</small></span><span v-if="cartCount" class="cart-badge nav-cart-badge">{{ cartCount > 99 ? '99+' : cartCount }}</span></Link>
                ...
                <div class="d-lg-none mobile-nav-panel"><form class="client-search-mobile" @submit.prevent="submitSearch"><i class="bi bi-search"/><input v-model="search" type="search" placeholder="Tìm kiếm sản phẩm..." aria-label="Tìm sản phẩm"/><button type="submit">Tìm</button></form><div class="mobile-nav-links"><Link href="/" @click="mobileMenuOpen = false"><i class="bi bi-house"/> Trang chủ</Link><Link href="/products?category=laptop" @click="mobileMenuOpen = false"><i class="bi bi-laptop"/> Laptop</Link><Link href="/products?category=components" @click="mobileMenuOpen = false"><i class="bi bi-pc-display"/> Linh kiện</Link><Link href="/products?category=monitor" @click="mobileMenuOpen = false"><i class="bi bi-display"/> Màn hình</Link><Link href="/products?category=accessories" @click="mobileMenuOpen = false"><i class="bi bi-keyboard"/> Phụ kiện</Link></div><div v-if="!authUser" class="mobile-auth-actions"><Link href="/login" class="nav-login-btn">Đăng nhập</Link><Link href="/register" class="nav-register-btn">Đăng ký tài khoản</Link></div></div>
            </div>
        </div>
    </nav>
