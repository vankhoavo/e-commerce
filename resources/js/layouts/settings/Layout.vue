...
import AddressCard from '@/components/settings/AddressCard.vue';
import Orders from '@/pages/settings/Orders.vue';
...
<template>
    <div class="settings-page-shell">
        ...
        <main class="settings-page-content">
            <slot />
            <AddressCard v-if="activeSection === 'profile'" />
...
body[data-techstore-settings-section="orders"] .settings-profile-block,body[data-techstore-settings-section="orders"] .settings-tab-panel,body[data-techstore-settings-section="orders"] .delete-account-wrap{display:none!important}
@media(max-width:1100px){.settings-nav-links{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:900px){.settings-nav-links{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:575px){.settings-page-shell{width:min(100% - 20px,1220px);padding-top:18px}.settings-hero{align-items:flex-start}.settings-hero-icon{width:46px;height:46px;flex-basis:46px}.settings-nav-links{grid-template-columns:1fr}}
</style>
