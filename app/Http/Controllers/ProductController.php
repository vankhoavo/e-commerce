...
class ProductController extends Controller
{
    ...
    {
        return AdminProduct::query()->with('category:id,name,slug');
    }
    ...
    private function transform(AdminProduct $p): array
    {
        ...
        return [
            'id' => $p->id, 'slug' => $p->slug, 'name' => $p->name, 'category' => $p->category?->name ?? 'Sản phẩm', 'categorySlug' => $p->category?->slug,
            'brand' => $p->brand ?? 'TechStore', 'price' => (int) $p->price, 'oldPrice' => $p->old_price ? (int) $p->old_price : null,
            'image' => $p->image, 'gallery' => $gallery, 'badge' => $p->badge, 'rating' => (float) $p->rating, 'sold' => (int) $p->sold_count,
            'stock' => (int) $p->stock, 'shortDescription' => $p->short_description, 'description' => $p->description, 'specs' => $p->specs ?: [],
        ];
    }
}
