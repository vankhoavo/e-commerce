...
function updateCountdown(): void {
    ...
    const remaining = Math.max(0, Math.ceil((new Date(expiresAt.value).getTime() - Date.now()) / 1000));
...
<template>
    ...
    <div class="forgot-card">
        ...
        <div v-if="page.props.status === 'otp-resent'" class="forgot-status resend-status" role="status">
            <i class="bi bi-check-circle-fill" />
            <span>Mã OTP mới đã được gửi đến Email của bạn.</span>
        </div>
        <div v-else-if="page.props.status === 'otp-verified'" class="forgot-status" role="status">
            <i class="bi bi-check-circle-fill" />
            <span>Mã OTP đã được xác thực. Đang chuyển sang bước đặt lại mật khẩu.</span>
        </div>
        <div v-else-if="page.props.status" class="forgot-status" role="status">
            <i class="bi bi-check-circle-fill" />
        ...
        <Form action="/settings/security/forgot-password/verify" method="post" v-slot="{ errors, processing }" class="forgot-form">
            <div>
                ...
                <p class="attempt-note">Bạn còn {{ remainingAttempts }} lần thử. Sai 3 lần liên tiếp, tài khoản sẽ bị khóa và quản trị viên sẽ được thông báo.</p>
            </div>
        ...
        <button type="button" class="resend-button" :disabled="showResend" @click="showResend = true">
            <i class="bi bi-arrow-clockwise" /> Gửi lại mã OTP
        </button>
        ...
        <Form v-if="showResend" action="/settings/security/forgot-password/verify/resend" method="post" v-slot="{ processing: resending }" class="resend-form">
            <button type="submit" class="resend-button secondary" :disabled="resending">
