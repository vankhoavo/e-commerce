...
// Custom date picker: no native browser calendar is used.
...
function moveMonth(step: number) {
    ...
    const max = new Date();
    if (next < min || next > new Date(max.getFullYear(), max.getMonth(), 1)) return;
...
// Current Vietnam two-level administrative structure: province/city -> ward/commune.
...
onMounted(async () => {
    ...
    try {
        ...
        if (province) {
            selectedProvince.value = String(province.code);
            await loadWards(selectedProvince.value);
            const savedWard = String(user.value.address_ward ?? '');
            const ward = wards.value.find((item) => item.name === savedWard);
            if (ward) selectedWard.value = String(ward.code);
        }
    } catch {
...
function selectedProvinceName() {
    return provinces.value.find((item) => String(item.code) === selectedProvince.value)?.name ?? '';
}

function selectedWardName() {
    return wards.value.find((item) => String(item.code) === selectedWard.value)?.name ?? '';
}
    ...
    <div class="settings-dashboard">
        <section v-show="activeSection === 'profile'" class="settings-view settings-profile-block">
            <div class="view-header">
                <div class="view-title-wrap"><div class="view-icon profile-icon"><UserRound :size="21" /></div><div><span class="section-kicker">TÀI KHOẢN</span><h2>Thông tin cá nhân</h2><p>Quản lý thông tin cá nhân, liên hệ và địa chỉ nhận hàng.</p></div></div>
                <Form action="/logout" method="post"><button type="submit" class="settings-logout"><i class="bi bi-box-arrow-right" /> Đăng xuất</button></Form>
            </div>
                    ...
                    <section class="settings-card">
                        ...
                        <Form v-bind="ProfileController.update.form()" v-slot="{ errors, processing }" class="settings-form">
                            ...
                            <div class="field date-field">
                                <label><CalendarDays :size="14" /> Ngày sinh</label>
                                <input type="hidden" name="birth_date" :value="birthDate" />
                                <div class="date-picker" :class="{ open: datePickerOpen }">
                                    <button type="button" class="date-trigger" @click="datePickerOpen = !datePickerOpen"><span :class="{ placeholder: !birthDate }">{{ dateLabel }}</span><CalendarDays :size="17" /></button>
                                    <div v-if="datePickerOpen" class="date-popover">
                                        <div class="date-popover-head"><button type="button" class="date-nav" :disabled="calendarYear === 1900 && calendarMonth === 0" @click="moveMonth(-1)"><ChevronLeft :size="17" /></button><strong>{{ monthNames[calendarMonth] }} {{ calendarYear }}</strong><button type="button" class="date-nav" :disabled="calendarYear === new Date().getFullYear() && calendarMonth === new Date().getMonth()" @click="moveMonth(1)"><ChevronRight :size="17" /></button></div>
                                        <div class="date-week"><span v-for="day in weekDays" :key="day">{{ day }}</span></div>
                                        <div class="date-grid"><button v-for="(day, index) in calendarCells" :key="index" type="button" :disabled="!day || isFutureDate(calendarYear, calendarMonth, day)" :class="{ selected: day && birthDate === `${calendarYear}-${String(calendarMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` }" @click="selectDate(day)">{{ day ?? '' }}</button></div>
                                        <div class="date-popover-foot"><button type="button" @click="clearBirthDate">Xóa</button><button type="button" @click="chooseToday">Hôm nay</button></div>
                                    </div>
                                </div>
                                <small class="field-hint">Chọn ngày sinh trong lịch.</small><InputError :message="errors.birth_date" />
                            </div>
                            ...
                            <div class="field full address-selector-card">
                                <label><MapPin :size="14" /> Địa chỉ nhận hàng</label>
                                <div class="address-grid">
                                    <div class="select-field"><span>Tỉnh / Thành phố</span><div class="select-wrap"><select name="address_province" v-model="selectedProvince" :disabled="addressLoading" @change="onProvinceChange"><option value="">{{ addressLoading ? 'Đang tải...' : 'Chọn tỉnh / thành phố' }}</option><option v-for="province in provinces" :key="province.code" :value="String(province.code)">{{ province.name }}</option></select><ChevronDown :size="15" /></div></div>
                                    <div class="select-field"><span>Phường / Xã</span><div class="select-wrap"><select name="address_ward" v-model="selectedWard" :disabled="!selectedProvince || addressLoading"><option value="">{{ selectedProvince ? 'Chọn phường / xã' : 'Chọn tỉnh trước' }}</option><option v-for="ward in wards" :key="ward.code" :value="String(ward.code)">{{ ward.name }}</option></select><ChevronDown :size="15" /></div></div>
                                </div>
                                <div class="field address-detail-field"><label>Số nhà, tên đường</label><input name="address_detail" v-model="addressDetail" maxlength="250" placeholder="Ví dụ: 25 Nguyễn Văn Linh" autocomplete="street-address" /></div>
                                <div class="address-preview"><MapPin :size="15" /><span>{{ addressDetail || 'Số nhà, tên đường' }}, {{ selectedWardName() || 'Phường / Xã' }}, {{ selectedProvinceName() || 'Tỉnh / Thành phố' }}</span></div>
                                <small class="field-hint">Địa chỉ được chọn từ danh sách, không cần nhập phường/xã hoặc tỉnh/thành phố bằng tay.</small>
                                <small v-if="addressError" class="field-error-text">{{ addressError }}</small>
                            </div>
        ...
        <section v-show="activeSection === 'security'" class="settings-view security-view">
            <div class="view-header"><div class="view-title-wrap"><div class="view-icon security-icon"><ShieldCheck :size="21" /></div><div><span class="section-kicker">BẢO MẬT TÀI KHOẢN</span><h2>Bảo mật</h2><p>Kiểm soát mật khẩu, xác thực hai lớp và phương thức đăng nhập.</p></div></div></div>
            <section class="settings-card password-card"><div class="card-heading"><span class="card-heading-icon"><LockKeyhole :size="18" /></span><div><h3>Đổi mật khẩu</h3><p>Nên sử dụng mật khẩu dài, riêng biệt và khó đoán.</p></div></div><Form v-bind="SecurityController.update.form()" :options="{ preserveScroll: true }" reset-on-success :reset-on-error="['password','password_confirmation','current_password']" v-slot="{ errors, processing }" class="settings-form"><div class="field full"><label>Mật khẩu hiện tại</label><PasswordInput name="current_password" autocomplete="current-password" placeholder="Nhập mật khẩu hiện tại" /><InputError :message="errors.current_password" /></div><div class="field"><label>Mật khẩu mới</label><PasswordInput name="password" autocomplete="new-password" placeholder="Nhập mật khẩu mới" :passwordrules="props.passwordRules" /><InputError :message="errors.password" /></div><div class="field"><label>Xác nhận mật khẩu</label><PasswordInput name="password_confirmation" autocomplete="new-password" placeholder="Nhập lại mật khẩu" :passwordrules="props.passwordRules" /><InputError :message="errors.password_confirmation" /></div><div class="password-footer"><div class="security-tip"><span><i class="bi bi-shield-check" /></span><div><strong>Mẹo bảo mật</strong><small>Kết hợp chữ hoa, chữ thường, số và ký tự đặc biệt.</small></div></div><button class="primary-button" :disabled="processing"><i class="bi bi-check2" />{{ processing ? 'Đang cập nhật...' : 'Cập nhật mật khẩu' }}</button></div></Form></section>
            <div class="security-grid"><section v-if="props.canManageTwoFactor" class="settings-card feature-card"><div class="feature-head"><span class="feature-icon blue"><ShieldCheck :size="19" /></span><div><h3>Xác thực 2 lớp</h3><p>Bảo vệ tài khoản bằng mã xác thực từ điện thoại.</p></div></div><div class="feature-content"><ManageTwoFactor :canManageTwoFactor="props.canManageTwoFactor" :requiresConfirmation="props.requiresConfirmation" :twoFactorEnabled="props.twoFactorEnabled" /></div></section><section v-if="props.canManagePasskeys" class="settings-card feature-card"><div class="feature-head"><span class="feature-icon purple"><KeyRound :size="19" /></span><div><h3>Passkey</h3><p>Đăng nhập không cần mật khẩu bằng thiết bị tin cậy.</p></div></div><div class="feature-content"><ManagePasskeys :canManagePasskeys="props.canManagePasskeys" :passkeys="props.passkeys" /></div></section></div>
        </section>
