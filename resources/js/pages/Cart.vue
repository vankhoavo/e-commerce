...
<aside class="col-lg-4"><div class="cart-summary-card"><div class="summary-title">Tóm tắt đơn hàng</div><div class="summary-row"><span>Tạm tính</span><strong>{{ formatPrice(subtotal) }}</strong></div><div class="summary-divider"/><div class="summary-total"><span>Tổng cộng</span><strong>{{ formatPrice(subtotal) }}</strong></div><Link href="/checkout" class="checkout-btn">Tiến hành đặt hàng <i class="bi bi-arrow-right"/></Link></div><div class="cart-service-card"><div><i class="bi bi-truck"/><span><strong>Giao hàng toàn quốc</strong><small>Đóng gói an toàn, theo dõi đơn.</small></span></div><div><i class="bi bi-shield-check"/><span><strong>Thanh toán an toàn</strong><small>Hỗ trợ COD và PayPal.</small></span></div><div><i class="bi bi-headset"/><span><strong>Hỗ trợ mua hàng</strong><small>Tư vấn trước và sau khi đặt hàng.</small></span></div></div></aside>
</div>
