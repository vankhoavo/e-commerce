    ...
    <div class="profile-page">
        ...
        <div class="row g-4">
            <div class="col-xl-4"><div class="profile-card profile-summary h-100">
                <div class="profile-avatar-wrap"><div class="profile-avatar"><img v-if="user.avatar" :src="user.avatar" alt="Ảnh đại diện"><i v-else class="bi bi-person-fill" /></div><span class="profile-online"><i class="bi bi-check" /></span></div>
                <h2>{{ user.name }}</h2><p class="profile-email">{{ user.email }}</p>
                <span class="profile-verified" :class="user.email_verified_at ? 'verified' : 'unverified'"><i :class="['bi', user.email_verified_at ? 'bi-patch-check-fill' : 'bi-exclamation-circle-fill']" />{{ user.email_verified_at ? 'Email đã xác thực' : 'Cần xác thực Email' }}</span>
                <Link v-if="!user.email_verified_at" href="/verify-email-otp" class="btn btn-sm btn-outline-primary verify-email-btn mt-2"><i class="bi bi-envelope-check me-1" />Xác thực Email ngay</Link>
                <div class="profile-summary-list mt-4"><div><i class="bi bi-person-badge" /><span><small>Vai trò</small><strong>{{ user.role?.label ?? user.role ?? 'Khách hàng' }}</strong></span></div><div><i class="bi bi-shield-check" /><span><small>Trạng thái</small><strong>{{ user.is_active ? 'Đang hoạt động' : 'Đã khóa' }}</strong></span></div></div>
            </div></div>
            <div class="col-xl-8">
                ...
                <section class="profile-card email-change-card mt-4"><div class="email-change-head"><div class="profile-title-icon email-icon"><i class="bi bi-envelope-at-fill" /></div><div><span class="section-kicker">BẢO MẬT EMAIL</span><h2>Địa chỉ Email</h2><p>Email hiện tại được dùng để đăng nhập và khôi phục tài khoản.</p></div></div>
