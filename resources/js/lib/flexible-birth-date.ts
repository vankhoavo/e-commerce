...
function displayDate(year: number, month: number, day: number): string {
    const suffix = year < 0 ? ' TCN' : '';
    return `${String(day).padStart(2, '0')}/${String(month + 1).padStart(2, '0')}/${Math.abs(year)}${suffix}`;
}

function currentDate(): { year: number; month: number; day: number } {
    const now = new Date();
...
function buildPicker(original: HTMLElement): void {
    ...
    wrapper.innerHTML = `
        <div class="flexible-birth-input-row">
            <input class="flexible-birth-input" type="text" inputmode="numeric" autocomplete="bday" placeholder="dd/mm/yyyy" aria-label="Ngày sinh">
            <button class="flexible-birth-open" type="button" aria-label="Mở lịch"><span>▦</span></button>
        </div>
        <div class="flexible-birth-popover" hidden>
            <div class="flexible-birth-head">
                <button class="flexible-birth-nav prev" type="button" aria-label="Tháng trước">‹</button>
                <div class="flexible-birth-period">
                    <select class="flexible-birth-month" aria-label="Chọn tháng"></select>
                    <input class="flexible-birth-year" type="number" inputmode="numeric" aria-label="Nhập năm" min="${MIN_YEAR}" max="${MAX_YEAR}" step="1">
                </div>
                <button class="flexible-birth-nav next" type="button" aria-label="Tháng sau">›</button>
            </div>
            <div class="flexible-birth-week"></div>
            <div class="flexible-birth-grid"></div>
            <div class="flexible-birth-foot">
                <button class="flexible-birth-clear" type="button">Xóa</button>
                <button class="flexible-birth-today" type="button">Hôm nay</button>
            </div>
        </div>`;
    ...
    function syncHidden(value: string): void {
        hidden.value = value;
    ...
    yearInput.addEventListener('change', () => {
        state.year = clampYear(Number(yearInput.value) || 0);
        yearInput.value = String(state.year);
        render();
    });
    ...
    wrapper.querySelector<HTMLButtonElement>('.clear')?.addEventListener('click', () => undefined);
