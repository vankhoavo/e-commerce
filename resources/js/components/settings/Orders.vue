...
function cancelOrder(order: TechStoreOrder) {
    if (!userId.value || order.status !== 'Chờ xử lý' || !window.confirm(`Bạn có chắc muốn hủy đơn ${order.code}?`)) return;
    orders.value = updateOrderStatus(userId.value, order.code, 'Hủy hàng');
}
function requestReturn(order: TechStoreOrder) {
    if (!userId.value || order.status !== 'Đã giao' || !window.confirm(`Gửi yêu cầu trả hàng cho đơn ${order.code}?`)) return;
...
@media print{
    ...
    .invoice-table tr{break-inside:avoid!important}
}
</style>
