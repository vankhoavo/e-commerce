...
class AdminController extends Controller
{
    ...
    {
        $year=now()->year;$ordersByMonth=array_fill(1,12,0);$revenueByMonth=array_fill(1,12,0);
        Order::query()->whereYear('created_at',$year)->get(['created_at','total'])->each(function(Order $order)use(&$ordersByMonth,&$revenueByMonth):void{$month=(int)$order->created_at->month;$ordersByMonth[$month]++;$revenueByMonth[$month]+=(int)$order->total;});
        return Inertia::render('admin/Dashboard',['stats'=>['orders'=>Order::count(),'revenue'=>(int)Order::sum('total'),'customers'=>User::query()->where('role',UserRole::CUSTOMER->value)->count(),'products'=>AdminProduct::count()],'charts'=>['months'=>array_map(fn(int $month):string=>'T'.$month,range(1,12)),'orders'=>array_values($ordersByMonth),'revenue'=>array_values($revenueByMonth)],'recentOrders'=>Order::query()->latest()->limit(6)->get(['id','code','customer_name','status','total','created_at'])]);
    }
