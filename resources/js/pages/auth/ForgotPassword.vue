...
defineOptions({
    layout: {
        ...
        description: 'Nhập Email để nhận mã OTP xác minh.',
    },
});
...
<template>
    ...
    <div class="forgot-card">
        ...
        <div v-if="status" class="forgot-status" role="status">
            <i class="bi bi-check-circle-fill" />
            <span>{{ status }}</span>
        </div>

        <Form action="/settings/security/forgot-password" method="post" v-slot="{ errors, processing }" class="forgot-form">
            <div>
...
<style scoped>
.forgot-card{width:100%;padding:26px;border:1px solid #e4e7ec;border-radius:18px;background:#fff;box-shadow:0 10px 30px rgba(16,24,40,.05)}
