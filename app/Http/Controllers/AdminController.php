...
class AdminController extends Controller
{
    public function dashboard(): Response
    {
        ...
        return Inertia::render('admin/Dashboard', [
            'stats' => ['orders' => Order::count(), 'revenue' => (int) Order::sum('total'), 'customers' => User::query()->where('role', UserRole::CUSTOMER->value)->count(), 'products' => AdminProduct::count()],
    ...
    public function inventory(): Response
    {
        ...
        $products=AdminProduct::query()->with('category:id,name')->orderBy('stock')->get()->map(fn(AdminProduct $product)=>['id'=>$product->id,'name'=>$product->name,'sku'=>$product->sku,'image'=>$product->image,'category'=>$product->category?->name??'Chưa phân loại','stock'=>(int)$product->stock,'sold'=>(int)($soldByProduct[$product->id]??0),'price'=>(int)$product->price,'is_active'=>(bool)$product->is_active]);
