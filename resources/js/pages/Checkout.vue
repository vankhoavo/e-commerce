...
type PaypalSdkInstance = {
    ...
    createPayPalOneTimePaymentSession: (options: Record<string, any>) => any;
};

declare global {
    interface Window {
        paypal?: { createInstance: (options: Record<string, any>) => Promise<PaypalSdkInstance> };
    }
}
...
const paypalSession = ref<any>(null);
...
async function capturePaypalOrder({ orderId }: { orderId: string }) {
...
async function setupPaypal() {
    ...
    try {
        ...
        paypalSession.value = sdk.createPayPalOneTimePaymentSession({
            ...
            onCancel: () => { paypalLoading.value = false; paypalError.value = 'Bạn đã hủy thanh toán PayPal Sandbox.'; },
            onError: (error: unknown) => { paypalLoading.value = false; console.error(error); paypalError.value = 'PayPal Sandbox gặp lỗi. Vui lòng thử lại.'; },
        });

        const button = document.createElement('paypal-button');
        button.setAttribute('type', 'pay');
        button.addEventListener('click', async () => {
            if (!validateShipping() || !paypalSession.value) return;
...
function selectPayment(payment: string) {
    form.value.payment = payment;
    if (payment === 'paypal') nextTick(setupPaypal);
}
                        ...
                        <div class="checkout-order-card">
                            ...
                            <div class="payment-block">
                                ...
                                <div v-if="form.payment === 'paypal'" class="paypal-sandbox-panel">
                                    ...
                                    <p class="sandbox-footnote"><i class="bi bi-lock-fill"/> TechStore tạo và capture PayPal Order ở máy chủ; Client Secret không đưa ra trình duyệt.</p>
                                </div>
                            </div>
