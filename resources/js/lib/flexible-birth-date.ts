...
function daysInMonth(year: number, month: number): number { if (month === 1) return isLeapYear(year) ? 29 : 28; return [3, 5, 8, 10].includes(month) ? 30 : 31; }
function isValidDate(year: number, month: number, day: number): boolean { return Number.isSafeInteger(year) && year >= MIN_YEAR && year <= MAX_YEAR && Number.isInteger(month) && month >= 0 && month <= 11 && Number.isInteger(day) && day >= 1 && day <= daysInMonth(year, month); }
function weekdayMondayFirst(year: number, month: number, day: number): number { let y = year; let m = month + 1; if (m < 3) { y -= 1; m += 12; } const k = mod(y, 100); const j = Math.floor(y / 100); const h = mod(day + Math.floor((13 * (m + 1)) / 5) + k + Math.floor(k / 4) + Math.floor(j / 4) + 5 * j, 7); return mod(h + 5, 7); }
function padYear(year: number): string { return `${year < 0 ? '-' : ''}${String(Math.abs(year)).padStart(4, '0')}`; }
...
function fromIso(value: string): CalendarDate | null { const match = value.trim().match(/^(-?\d+)-(\d{1,2})-(\d{1,2})$/); if (!match) return null; const year = Number(match[1]); const month = Number(match[2]) - 1; const day = Number(match[3]); return isValidDate(year, month, day) ? { year, month, day } : null; }
function parseDisplay(value: string): CalendarDate | null { const cleaned = value.trim().replace(/\s+/g, ' '); const match = cleaned.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](-?\d+)(?:\s*(TCN|BCE))?$/i); if (!match) return fromIso(cleaned); const day = Number(match[1]); const month = Number(match[2]) - 1; let year = Number(match[3]); if (/^(TCN|BCE)$/i.test(match[4] ?? '')) year = -Math.abs(year); return isValidDate(year, month, day) ? { year, month, day } : null; }
...
function currentDate(): CalendarDate { const now = new Date(); return { year: now.getFullYear(), month: now.getMonth(), day: now.getDate() }; }
function clampYear(year: number): number { if (!Number.isFinite(year)) return 0; return Math.max(MIN_YEAR, Math.min(MAX_YEAR, Math.trunc(year))); }
...
function buildPicker(original: HTMLElement): void {
    ...
    const legacyPopover = original.querySelector<HTMLElement>('.date-popover');
    legacyPopover?.style.setProperty('display', 'none', 'important');
    ...
    const syncHidden = (value: string): void => { hidden.value = value; hidden.dispatchEvent(new Event('input', { bubbles: true })); hidden.dispatchEvent(new Event('change', { bubbles: true })); };
    const syncInput = (): void => { input.value = selected ? displayDate(selected.year, selected.month, selected.day) : ''; syncHidden(selected ? toIso(selected.year, selected.month, selected.day) : ''); };
    ...
    const open = (): void => { commitTypedValue(); popover.hidden = false; render(); };

    syncInput();
    input.addEventListener('input', () => input.setCustomValidity(''));
    input.addEventListener('change', commitTypedValue);
    input.addEventListener('blur', commitTypedValue);
    input.addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); commitTypedValue(); open(); } if (event.key === 'Escape') popover.hidden = true; });
    ...
    yearInput.addEventListener('change', () => { state.year = clampYear(Number(yearInput.value)); render(); });
